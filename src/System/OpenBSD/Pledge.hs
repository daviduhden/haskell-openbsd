...
-}
data Promise
    = -- | audio(4) device @ioctl@ operations
      Audio
    | -- | bpf(4) statistics collection (@BIOCGSTATS@)
      Bpf
    | -- | change file owner or group
      Chown
    | -- | create or remove files
      Cpath
    | -- | disk label @ioctl@ operations
      Disklabel
    | -- | DNS resolution
      Dns
    | -- | create special files (@mkfifo(2)@, @mknod(2)@)
      Dpath
    | -- | drm(4) @ioctl@ operations
      Drm
    | -- | return @ENOSYS@ on violations instead of being killed
      Error
    | -- | execute processes with @execve(2)@
      Exec
    | -- | change @struct stat@ fields of files
      Fattr
    | -- | file locking
      Flock
    | -- | read-only password database access
      Getpw
    | -- | change process rights (@setuid@, @setgroups@, ...)
      Id
    | -- | @AF_INET@\/@AF_INET6@ socket operations
      Inet
    | -- | multicast socket operations (used with 'Inet')
      Mcast
    | -- | pf(4) @ioctl@ operations
      Pf
    | -- | process relationship operations (@fork@, @kill@, ...)
      Proc
    | -- | use @PROT_EXEC@ with @mmap(2)@\/@mprotect(2)@
      ProtExec
    | -- | process listing @sysctl(2)@ interfaces
      Ps
    | -- | receive file descriptors with @recvmsg(2)@
      Recvfd
    | -- | inspect the routing table
      Route
    | -- | read-only filesystem operations
      Rpath
    | -- | send file descriptors with @sendmsg(2)@
      Sendfd
    | -- | set system time
      Settime
    | -- | basic memory, fd, timer and process-attribute operations
      Stdio
    | -- | tape drive @ioctl@ operations
      Tape
    | -- | tty @ioctl@ operations and @\/dev\/tty@ access
      Tty
    | -- | @AF_UNIX@ socket operations
      Unix
    | -- | allow @unveil(2)@ to be called
      Unveil
    | -- | video(4) @ioctl@ operations
      Video
    | -- | virtual-memory inspection (@top(1)@, @vmstat(8)@)
      Vminfo
    | -- | vmm(4) @ioctl@ operations
      Vmm
