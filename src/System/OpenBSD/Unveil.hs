...
-}
data Permission
    = -- | \"r\": read operations (pledge @rpath@)
      Read
    | -- | \"w\": write operations (pledge @wpath@, @chown@, @fattr@)
      Write
    | -- | \"x\": execute operations (pledge @exec@)
      Execute
    | -- | \"c\": create and remove operations (pledge @cpath@, @dpath@, @unix@)
      Create
